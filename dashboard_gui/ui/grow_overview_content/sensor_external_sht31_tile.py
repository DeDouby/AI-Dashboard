...
class SensorExternalSHT31Tile(BoxLayout):
    ...
    def update_values(self, data):
        # Wir greifen nun auf das 'external' Objekt zu
        external = data.get("external", {})
    
        # Sicherstellen, dass wir die Werte aus dem Dictionary holen (analog zu deiner Pipeline)
        temp_data = external.get("temperature", {})
        hum_data  = external.get("humidity", {})
        vpd_data  = data.get("vpd_external", {}) # VPD External kommt laut deiner Pipeline direkt aus stream
        ...
        # WICHTIG: Die Trend-Keys müssen auf 'ex' (External) enden
        trend_temp = GLOBAL_STATE.get_trend_icon("temp_ex")
        trend_hum  = GLOBAL_STATE.get_trend_icon("hum_ex")
        trend_vpd  = GLOBAL_STATE.get_trend_icon("vpd_ex")
