    ...
    def _on_slider_change(self, instance, value):
            ...
            if mac:
                ...
                WEB_CLIENT.send_control(mac, {
                    "circulation_fan_pct": int(self.slider_max.value),
                    "circulation_fan_min": int(self.slider_min.value),
                    "rev": new_rev
                })
...
# --- ZUSÄTZLICHE OPTIMIERUNG DER UPDATE-LOGIK ---
    def update_ui(self, *_):
        ...
        # --- WERTE ÜBERNEHMEN ---
        arduino_mode = server_data.get('circulation_fan_mode', 'man')
        arduino_min = server_data.get('circulation_fan_min', 0)
        arduino_max = server_data.get('circulation_fan_pct', 0)
        ...
        self.lbl_rpm.text = f"RPM: {server_data.get('circulation_fan_rpm', 0)}"
