...
class UIFormatter:
    @staticmethod
    def format_sensor_label(name, value, unit, trend="", sz_val=42, sz_trend=18, sz_unit=16, sz_name=16):
        C_SUB = "#bbbbbb"
        
        # WICHTIG: Wir skalieren die Werte HIER für das Markup
        # Wir wandeln in int um, da Markup keine Nachkommastellen bei size mag
        s_sz_name = int(sp_scaled(sz_name))
        s_sz_trend = int(sp_scaled(sz_trend))
        s_sz_val = int(sp_scaled(sz_val))
        s_sz_unit = int(sp_scaled(sz_unit))
