...
class ExhaustFanOverlay(FloatLayout):
    def __init__(self, parent_header, **kwargs):
        ...
        # Wert-Anzeigen
        self.lbl_val = Label(text="0% - 0%", font_size=sp_scaled(30), bold=True, size_hint_y=None, height=dp_scaled(30))
        ...
        self.temp_slider = UnifiedSlider(range_min=15, range_max=35, min=22, max=28, mode='range', size_hint_y=None, height=dp_scaled(35))
        ...
        self.vpd_slider = UnifiedSlider(min=8, max=15, range_min=0, range_max=30, mode='range', size_hint_y=None, height=dp_scaled(35))
        ...
        self.panel.add_widget(Widget()) # Spacer
    ...
    def _force_sync(self, *_):
        self._send_command("exhaust_fan", mode=self._target_mode)
    ...
    def _set_mode(self, mode):      
        ...
        self._send_command("exhaust_fan", mode=mode)
...
# ... (Layout bleibt gleich)
    ...
    def update_ui(self, *_):
        ...
        # SESSION-CHECK (Stoppt Sync, wenn ESP32 neu gestartet ist)
        ...
        if not hasattr(self, "_last_adopted_init") or self._last_adopted_init != current_init_rev:
            ...
            # Optional: Hier initial_values() triggern, falls nötig
            return
        ...
        # Wir sind synced, wenn Server-Rev >= unsere Rev UND User-Pause > 1.8s
        is_synced = (server_rev >= last_sent) and not self._user_active and (time_since_action > 1.8)

        # Live-Werte (Hardware-Feedback) - IMMER updaten
        ...
        if not is_synced:
            # STATUS ORANGE: User editiert gerade oder Paket ist noch im Flug
        ...
        if not self._user_active:
            # WICHTIG: UI sperren, damit die Slider-Binds nicht feuern!
            self._ui_lock = True
            self._apply_server_snapshot(server_data)
            self._ui_lock = False

    def _apply_server_snapshot(self, data):
        """Übernimmt alle Serverwerte in die UI-Elemente."""
        # Speed & Mode
        ...
        # Slider setzen (Erst Max dann Min wegen Kivy-Validierung)
        self._ui_lock = True
        ...
        # Revision-Sync: Wir akzeptieren den Stand des Servers als unseren letzten Stand
    ...
    def _on_slider_change(self, *args):
        if not self._init_done or self._ui_lock: 
            return
        if not self._init_done or self._locked or getattr(self, "_ui_lock", False):
            return
    ...
    def _touch_up(self, instance, touch):
        if self._user_active:
            ...
            self._send_command("exhaust_fan_range")

    def _on_unlock(self):
        self._locked = False
        for s in [self.range_slider, self.temp_slider, self.hum_slider, self.vpd_slider]: s.disabled = False
