...
class LightOverlay(FloatLayout):
    def __init__(self, parent_header, **kwargs):
        ...
        self._last_send_time = 0          # NEU: explizit für Timeout-Tracking
        self._retry_count = 0             # NEU: Zähler für Auto-Retries
        self._max_retries = 5             # NEU: max. 5 Versuche bei Netzwerk-Hang
        ...
        val_box.add_widget(self.lbl_target)
    ...
    # ========================================================
    # ZENTRALE LOGIK: TARGET-REVISION + VERBESSERTER AUTO-SYNC
    # ========================================================
    def update_ui(self, *_):
        ...
        # 2. REVISION + AUTO-RETRY LOGIK (JETZT KORREKT IMPLEMENTIERT)
        ...
        # === AUTO-RETRY BEI NETZWERK-HANG ===
        if pending and (time.time() - last_send_time > 3.0):
            retry_count = getattr(self, '_retry_count', 0)
            if retry_count < self._max_retries:
                self._retry_count = retry_count + 1
                print(f"NETWORK TIMEOUT: Resending Rev {last_sent} (Retry {self._retry_count}/{self._max_retries})")
                self._send_command(is_retry=True)   # explizit als Retry markiert
                return  # Frame abbrechen – nächster Poll prüft neu
            else:
                print(f"MAX RETRIES REACHED ({self._max_retries}) for Rev {last_sent} – Network issue?")

        # LIVE FEEDBACK (immer aktuell, auch während Pending/Retries)
        eff_pct = server_data.get('light_pct', 0)
        target_pct = server_data.get('light_target', 0)
        self.lbl_val.text = f"{int(eff_pct)}%"
        self.lbl_target.text = f"({int(target_pct)}%)"
        self.lbl_val.color = (1, 0.8, 0, 1) if abs(eff_pct - target_pct) > 0.5 else (1, 1, 1, 1)

        # STATUS-ICON
        if not is_synced:
            if pending and getattr(self, '_retry_count', 0) >= self._max_retries:
                # Warnung nach max. Versuchen
        ...
        # === STATUS GRÜN (SYNCED) ===
        self._retry_count = 0                     # Erfolg → Zähler zurücksetzen
        self.sync_icon.text = "[font=FA]\uf058[/font]"
    ...
    def _send_command(self, is_retry=False, **kwargs):
        """Sendet Kommando und setzt Retry-Logik nur bei user-initiierten Sends zurück"""
        mac = GLOBAL_STATE.get_active_device_id()
        if not mac or not self._init_done: 
            return
        ...
        if rev:
            ...
            # WICHTIG: Nur bei echten User-Aktionen (nicht bei Auto-Retry) Zähler zurücksetzen
            if not is_retry:
                self._retry_count = 0
    ...
    def _touch_down(self, slider, touch):
        if not self._locked and slider.collide_point(*touch.pos): 
    ...
    def _touch_up(self, slider, touch):
        if self._user_active:
            ...
            self._send_command()          # user-initiiert → is_retry=False (default)

    def _set_mode(self, mode):
        if not self._locked: 
            self._send_command(mode=mode)  # user-initiiert

    def _on_unlock(self):
        self._locked = False
        for s in [self.slider, self.slider_start, self.slider_dur, self.slider_sunrise_sunset]: 
            s.disabled = False

    def _force_sync(self, *_): 
        self._send_command()              # manuelle Force-Sync → auch is_retry=False
    ...
    def _init_values(self, *_):
        ...
        self._retry_count = 0                     # Init sauber
    ...
    def close(self):
        ...
        if self._sync_event: 
            self._sync_event.cancel()          # sauber mit aufräumen
        if self.parent: 
