...
class DataFlowEngine:
    ...
    def process_cycle(self):
        ...
        if not self.gsm.running or not data:
            if not data: self.gsm.led_engine.offline()
            return
        ...
        for device_frame in data:
            ...
            # Alle Kanäle (adv, gatt, webserver) sind nun im Decoder-Frame gleichwertig
            for ch_type in ("adv", "gatt", "webserver"):
                ch_data = device_frame.get(ch_type, {})
                if isinstance(ch_data, dict) and ch_data.get("alive"):
                    # Metrics & Hintergrund-Logging
            ...
            # Background RSSI & Timestamps
            self._update_background_rssi(dev_id, device_frame)

        # Mixed Mode Update (Global für alle Daten)
        ...
        # Den für die UI relevanten Kanal-Slot greifen
        ch = d.get(ch_name, {})
        d["channel"] = ch_name 
        ...
        # Health & LED (basiert auf dem gewählten Kanal)
    ...
    def _update_background_rssi(self, dev_id, frame):
        try:
            ...
            # RSSI History (Wichtig für den Signal-Graphen)
            ...
            print(f"[DFE] Background Update Error: {e}")

    def _handle_health_and_leds(self, d, ch, ch_name, dev_id):
        # Wenn der gewählte Kanal nicht alive ist -> LED auf Rot
        if not ch.get("alive", False):
            self.gsm.led_engine.offline()
            return

        # --- FLOW ERKENNUNG (LED Animation) ---
        if ch_name == "webserver":
            # Zeitstempel-Vergleich (Daten kommen vom Decoder)
