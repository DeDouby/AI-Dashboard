...
ExhaustDecision compute_exhaust_decision(const ExhaustInputs& in) {
    ...
    // 3. & 4. AUTOMATIC MODE (Failsafe + Normal Regulation)
    else {
        // --- Normal Regulation (Basis-Berechnung) ---
        ...
        // Reason_1 Auswertung für Normalbetrieb
        ...
        // --- Failsafe Conditions (Überschreibt Normal Regulation bei Bedarf) ---
    ...
    if (in.chaos_active) {
        ...
        // Reason_2 Fallback für den Manual Mode, falls Chaos aktiv ist
...
void exhaust_fan_update() {
    ...
    inputs.out_temp = BLEScanner::get_sps_temp();
    inputs.out_hum  = BLEScanner::get_sps_hum();
    inputs.out_ok   = BLEScanner::is_sps_online();
