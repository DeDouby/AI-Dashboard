...
void SystemReset::perform_factory_reset() {
    Serial.println("\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    Serial.println("[RESET] FACTORY RESET AUSGELÖST! LÖSCHE ALLES...");
    Serial.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");

    Preferences prefs;
    
    // 1. Grow-Controller Settings löschen
    prefs.begin("grow", false);
    prefs.clear();
    prefs.end();
    Serial.println("[RESET] 'grow' Preferences gelöscht.");

    // INFO: Wenn du später "light", "fan" etc. hast, einfach hier erweitern:
    // prefs.begin("light", false); prefs.clear(); prefs.end();

    Serial.println("[RESET] Alle Daten bereinigt. Neustart in 2 Sekunden...");
    delay(2000);
    ESP.restart();
}
