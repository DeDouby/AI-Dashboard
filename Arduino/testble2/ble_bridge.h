...
public:
    ...
    void updateBroadcast(float t_ext, float h_ext, float t_int, float h_int, float t_leaf, float v_bat, int rpm) {
        ...
        // Bestehende Sensor-Logik (wie gehabt)
        ...
        // --- NEU: RPM verpacken auf Pos 14 & 15 ---
        uint16_t r = (uint16_t)constrain(rpm, 0, 30000);
        payload[14] = (r >> 8) & 0xFF; 
        payload[15] = r & 0xFF;
        
        // PacketCounter auf die letzte Position (16)
        ...
        for(int i = 0; i < 17; i++) { // Wichtig: Loop bis 17!
            mData += (char)payload[i];
        }
        oAdvertisementData.setManufacturerData(mData);
        oScanResponseData.setName(BRIDGE_NAME);
