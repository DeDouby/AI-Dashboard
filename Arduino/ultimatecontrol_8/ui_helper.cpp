...
// --- FAN TILE ---
    ...
    // In deinem UI Setup für den fan_slider:
    lv_obj_t * fan_slider = lv_slider_create(tile_fan);
    lv_obj_set_size(fan_slider, 240, 25);
    lv_obj_align(fan_slider, LV_ALIGN_CENTER, 0, 0);
    
    // RANGE MODUS AKTIVIEREN
    lv_slider_set_mode(fan_slider, LV_SLIDER_MODE_RANGE); 
    lv_slider_set_range(fan_slider, 0, 100);
    
    // Startwerte: Links (Min) auf 20, Rechts (Max) auf 60
    lv_slider_set_value(fan_slider, 60, LV_ANIM_OFF);      // Der Hauptwert (Max)
    lv_slider_set_left_value(fan_slider, 20, LV_ANIM_OFF); // Der linke Wert (Min)
    
    lv_obj_add_event_cb(fan_slider, [](lv_event_t * e) {
        lv_obj_t * slider = lv_event_get_target(e);
        current_fan_speed = lv_slider_get_value(slider);      // Rechter Griff
        current_fan_min_speed = lv_slider_get_left_value(slider); // Linker Griff
        
        // UI Label Update
        lv_label_set_text_fmt(ui_fan_speed_label, "Range: %d - %d %%", 
                              current_fan_min_speed, current_fan_speed);
