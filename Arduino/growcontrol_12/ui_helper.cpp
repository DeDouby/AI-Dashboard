...
lv_obj_t * tile_circulation_fan;           // <--- HIER EINFÜGEN
lv_obj_t * ui_circulation_fan_speed_label;  // <---
lv_obj_t * ui_main_humid_label;  // <--- DIESE ZEILE HIER EINFÜGEN
// In ui_helper.cpp oben bei den anderen Definitionen:
...
void setup_ui() {
    ...
    tile_circulation_fan      = lv_tileview_add_tile(tileview, 2, 0, LV_DIR_LEFT | LV_DIR_BOTTOM); 
    ...
    add_bg(tile_circulation_fan,      &bild_1);
...
// --- circulation_fan TILE ---
    tile_circulation_fan = lv_tileview_add_tile(tileview, 2, 0, LV_DIR_ALL); 
    
    lv_obj_t * circulation_fan_title = lv_label_create(tile_circulation_fan);
    lv_label_set_text(circulation_fan_title, "PWM circulation_fan CONTROL (GPIO 4)");
    lv_obj_set_style_text_font(circulation_fan_title, &lv_font_montserrat_16, 0);
    lv_obj_set_style_text_color(circulation_fan_title, lv_color_hex(0x00FF88), 0);
    lv_obj_align(circulation_fan_title, LV_ALIGN_TOP_MID, 0, 10);
    
    // Speed & RPM Label
    ui_circulation_fan_speed_label = lv_label_create(tile_circulation_fan);
    lv_label_set_text(ui_circulation_fan_speed_label, "60 % | 0 RPM"); 
    lv_obj_set_style_text_color(ui_circulation_fan_speed_label, lv_color_hex(0x00FFFF), 0); 
    lv_obj_set_style_text_font(ui_circulation_fan_speed_label, &lv_font_montserrat_22, 0);
    lv_obj_align(ui_circulation_fan_speed_label, LV_ALIGN_CENTER, 0, -40);

    // --- 1. BUTTONS ERSTELLEN ---
    btn_nat = lv_btn_create(tile_circulation_fan);
    ...
    btn_chao = lv_btn_create(tile_circulation_fan);
    ...
    lv_obj_add_event_cb(btn_nat, [](lv_event_t * e) {
        ...
        if(lv_obj_has_state(obj, LV_STATE_CHECKED)) {
            circulation_fan_set_mode(circulation_fan_MODE_NATURAL);
            lv_obj_clear_state(btn_chao, LV_STATE_CHECKED); 
        } else {
            circulation_fan_set_mode(circulation_fan_MODE_MANUAL);
        }
    }, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(btn_chao, [](lv_event_t * e) {
        lv_obj_t * obj = lv_event_get_target(e);
        if(lv_obj_has_state(obj, LV_STATE_CHECKED)) {
            circulation_fan_set_mode(circulation_fan_MODE_CHAOTIC);
    ...
    // In deinem UI Setup für den circulation_fan_slider:
    lv_obj_t * circulation_fan_slider = lv_slider_create(tile_circulation_fan);
    lv_obj_set_size(circulation_fan_slider, 240, 25);
    lv_obj_align(circulation_fan_slider, LV_ALIGN_CENTER, 0, 0);
    
    // RANGE MODUS AKTIVIEREN
    lv_slider_set_mode(circulation_fan_slider, LV_SLIDER_MODE_RANGE); 
    lv_slider_set_range(circulation_fan_slider, 0, 100);
    
    // Startwerte: Links (Min) auf 20, Rechts (Max) auf 60
    lv_slider_set_value(circulation_fan_slider, 60, LV_ANIM_OFF);      // Der Hauptwert (Max)
    lv_slider_set_left_value(circulation_fan_slider, 20, LV_ANIM_OFF); // Der linke Wert (Min)
    
    lv_obj_add_event_cb(circulation_fan_slider, [](lv_event_t * e) {
        lv_obj_t * slider = lv_event_get_target(e);
        current_circulation_fan_speed = lv_slider_get_value(slider);      // Rechter Griff
        current_circulation_fan_min_speed = lv_slider_get_left_value(slider); // Linker Griff
        
        // UI Label Update
        lv_label_set_text_fmt(ui_circulation_fan_speed_label, "Range: %d - %d %%", 
                              current_circulation_fan_min_speed, current_circulation_fan_speed);
        
        // Bei manueller Änderung Modi ausschalten
        lv_obj_clear_state(btn_nat, LV_STATE_CHECKED);
        lv_obj_clear_state(btn_chao, LV_STATE_CHECKED);
        circulation_fan_set_mode(circulation_fan_MODE_MANUAL); 
...
// ================= INFO TILE BATT§ERY =================
    ...
    lv_obj_align(ui_battery_label, LV_ALIGN_CENTER, 0, 0); 
     ...
     // In der UI-Initialisierung:
    ui_time_label = lv_label_create(tile_info);
    lv_obj_set_style_text_font(ui_time_label, &lv_font_montserrat_24, 0); // Etwas kleiner als Batterie
    lv_obj_set_style_text_color(ui_time_label, lv_color_hex(0xFFFFFF), 0); // Weiß
    lv_obj_align(ui_time_label, LV_ALIGN_CENTER, 0, 60); // 60 Pixel unter der Batterie-Anzeige
