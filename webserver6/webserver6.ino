...
void loop() {
    ...
    if (millis() - last_update > 500) {
        ...
        // Lüfter RPM Update
        // --- Lüfter RPM Update ---
        int real_rpm = fan_get_rpm();
        if(ui_fan_speed_label) {
            if(real_rpm > 50) {
                lv_label_set_text_fmt(ui_fan_speed_label, "Speed: %d %% (%d RPM)", current_fan_speed, real_rpm);
            } else {
                lv_label_set_text(ui_fan_speed_label, "Fan: IDLE / OFF");
            }
        }
        update_battery_ui();
    }
    ...
    if (millis() - last_ble_update > 5000) {
        ...
        bleBridge.updateBroadcast(getTempExt(), getExternalHumidity(), getTempIn(), 40.0, 25.5f, get_battery_voltage_now(), fan_get_rpm());
    }
    if (is_display_off) {
            // Wir lassen den Loop schnell laufen, damit WebModule::update() 
            // oft genug aufgerufen wird!
            delay(10); 
        } else {
            delay(5);
        }
