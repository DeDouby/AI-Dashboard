...
def _safe_load_dump():
    """
    Lädt ble_dump.json und normalisiert auf:
    {
      "<address>": { ... }
    }
    """
    ...
    # Altes Format: list → konvertieren
    if isinstance(data, list):
        ...
        for entry in data:
            ...
            devices[addr] = {
                ...
                "adv_raw": entry.get("adv_raw", entry.get("raw", "")),
                "gatt_raw": None,
                "log_raw": entry.get("log_raw", entry.get("adv_raw", entry.get("raw", ""))),
            }
        return devices
...
class GattEngine:
    ...
    async def _run_async(self):
        ...
        self._log(f"[ENGINE] Notify-UUID: {notify_uuid}")
        
        if cmd_uuid and cmd_bytes:
            self._log(f"[ENGINE] Command-UUID: {cmd_uuid}, Command: {cmd_hex}")
        else:
            self._log("[ENGINE] Kein Command → reines Notify.")
        ...
        try:
            async with BleakClient(matched_dev) as client:
                ...
                # -------------------------------
                # Notify-Callback (ANDROID-PARITY)
                # -------------------------------
                ...
                self._log("[ENGINE] Lausche auf Notifications…")
                ...
                while self.running:
                    ...
                    # alle ~1s einen Command senden (5 × 0.2s)
                    if poll_tick >= 5:
                        poll_tick = 0
                        if cmd_uuid and cmd_bytes:
                            try:
...
def load_devices_from_dump():
    """
    Liest einmal data/ble_dump.json.
    Erwartet das neue Dict-Format (oder konvertiert das alte List-Format).
    Gibt Liste von (label, address, name) zurück.
    """
    devices = _safe_load_dump()
    result = []
